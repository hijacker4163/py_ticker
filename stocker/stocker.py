...
class Stocker():
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        # Actual observations
        ax.plot(train['ds'], train['y'], 'ko', ms = 4, label = '歷史資料')
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        ax.plot(stock_history['ds'], stock_history['y'], 'ko-', linewidth = 1.4, alpha = 0.8, ms = 1.8, label = '歷史資料')
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if not nshares:
            ...
            # Plot the actual values
            ax.plot(train['ds'], train['y'], 'ko-', linewidth = 1.4, alpha = 0.8, ms = 1.8, label = '歷史資料')
            ax.plot(test['ds'], test['y'], 'ko-', linewidth = 1.4, alpha = 0.8, ms = 1.8, label = '歷史資料')
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            plt.plot(future['ds'], future['yhat'], color = 'navy', linewidth = 2.0, label = '預測線')
